...
func (a *AndroidApp) generateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if mica := String(a.overridableAppProperties.Lineage); mica != "" {
		micaFile = android.PathForModuleSrc(ctx, mica)
	}
	rotationMinSdkVersion := String(a.overridableAppProperties.RotationMinSdkVersion)

	CreateAndSignAppPackage(ctx, packageFile, packageResources, jniJarFile, dexJarFile, certificates, apkDeps, v4SignatureFile, micaFile, rotationMinSdkVersion)
	...
	for _, split := range a.aapt.splits {
		...
		CreateAndSignAppPackage(ctx, packageFile, split.path, nil, nil, certificates, apkDeps, v4SignatureFile, micaFile, rotationMinSdkVersion)
